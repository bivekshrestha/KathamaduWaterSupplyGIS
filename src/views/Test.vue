              ...
              >
                ...
                <template v-if="selection.layers.length > 0 && !loading">
                  <template v-for="layer in selection.layers">
                    <vl-layer-vector :opacity="0.8" :key="String(layer.id) + String(layer.slug)" :z-index="5">
                      <vl-source-vector>
                        <vl-feature
...
import TotalCount from "../components/sections/landing/TotalCount";
...
import ManageLayersModal from "../components/modals/ManageLayersModal";
...
export default {
  ...
  methods: {
    ...
    async onGeotiffLoaded(layer) {
      ...
      const minValue = Math.min(...uniquePixelData);
      const maxValue = Math.max(...uniquePixelData);
